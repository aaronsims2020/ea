...
public class EncryptedRuleBuilder 
{
    ...
    /** Encrypt a file and return the encrypted object byte array. Rules file must be initialized. */
    public synchronized static byte[] encryptByteArray(byte[] byteArrayToEncrypt)
    {
        ...
        try
        {
            if(byteArrayToEncrypt!=null)
            {
                try
                {
                    ...
                    try
                    {
                        bais = new ByteArrayInputStream(byteArrayToEncrypt);
                        fos = new ByteArrayOutputStream(byteArrayToEncrypt.length);
                        cos1 = new CipherOutputStream(fos, cenc1);
                        //
                        int numRead = 0;
    ...
    private synchronized byte[] getJarFileEntryByteArrayAndDecrypt(String strJarEntry)
    {
        try 
        {
    ...
    public synchronized static byte[] getImage(String strJarEntry)
    {
        try 
        {
            ByteArrayOutputStream out = new ByteArrayOutputStream();
            InputStream is = out.getClass().getResource(strJarEntry).openStream();
            int c;    
            while((c=is.read())>=0) out.write(c);   
            is.close();
            byte[] theByteArray = out.toByteArray();
    ...
    private synchronized static byte[] decryptByteArray(byte[] byteArrayToDecrypt)
    {
        final byte[] buf = new byte[1024];  
        try
        {
