        ...
        try
        {
            ...
            if(propFile.exists()==true)
            {
                    try
                    {
                       ...
                       try
                        {
                            if(get("i18nBundleName")!=null)
                            {
                                if(get("i18nBundleName").equalsIgnoreCase("")==false)
                                {                
                                    setLocale(EncryptedRuleReader.get("i18nBundleName"));
                ...
                if(get("autoUpdateEnabled").equalsIgnoreCase("true")==true)
                {
                    if(get("autoUpdateUpdateInProgress")!=null)
                    {
                        if(get("autoUpdateUpdateInProgress").equalsIgnoreCase("true")==true)
                        {
                            ...
                            props.put("autoUpdateMaxAttempts","0");                             
                            writeFile();
                            return true;
                        }
                        return false;
...
/*    public static synchronized byte[] getImage(String strJarEntry)
    {
        try 
        {
