/*
 * MessagingEnabledControl.java
 *
 * Created on April 9, 2004, 9:24 PM
 */

package com.trinity.ea.design.messaging;

import com.trinity.ea.design.common.panel.EAPanel;
import com.trinity.ea.design.common.file.ProjectManager;
import java.awt.Color;
import java.awt.BorderLayout;
import javax.swing.JCheckBox;
import javax.swing.JPanel;

/** 
 *
 * @author  aaronsc Trinity Software
 * Copyright ©2003-2004 Trinity Software. All rights reserved.
 */
public class MessagingEnabledControl extends EAPanel {
    private Color backgroundColor = new java.awt.Color(100, 120, 170);
    private Color txtColor = new java.awt.Color(0, 0, 0);    
    
    /** Creates new form MessagingEnabledControl */
    public MessagingEnabledControl() {
        initComponents();
        setProjectData();
    }

    public synchronized void setProjectData()
    {
        try
        {
		// Messaging Support Functionality Checkbox Enabled/Disabled   
            if(ProjectManager.get("project_messaging_support_is_enabled").equalsIgnoreCase("true")==true)
            {
                cbMessagingEnabled.setSelected(true);              
            }
            else
            {
                cbMessagingEnabled.setSelected(false);               
            }
        }
        catch(Exception e)
        {
            e.printStackTrace();
        }
    }
  
    public synchronized void getDataUpdate()
    {
	try
	{
		// Messaging Support Functionality Checkbox Enabled/Disabled
            if(cbMessagingEnabled.isSelected()==true)
            {
		    ProjectManager.putTempNoFileWrite("msgEnabled", "true");
 		    ProjectManager.putTempNoFileWrite("project_messaging_support_is_enabled", "true");                            
            }
            else
            {
		    ProjectManager.putTempNoFileWrite("msgEnabled", "false");
		    ProjectManager.putTempNoFileWrite("project_messaging_support_is_enabled", "false");               
            }
            ProjectManager.saveTempNow();
        }
        catch(Exception e)
        {
            e.printStackTrace();
        }
    }
 
    public void setText(String cbText)
    {
        try
        {
            cbMessagingEnabled.setText(cbText);
        }
        catch(Exception e)
        {
            e.printStackTrace();
        }
    }
    
    public String getText()
    {
        try
        {
            return cbMessagingEnabled.getText();
        }
        catch(Exception e)
        {
            e.printStackTrace();
        }
        return null;
    }
    
    public void setBackgroundColor(Color BGColor)
    {
        try
        {
            backgroundColor = BGColor;
            cbMessagingEnabled.setBackground(BGColor);
            DescriptorPanel.setBackground(BGColor);
        }
        catch(Exception e)
        {
            e.printStackTrace();
        }      
    }
    
    public Color getBackgroundColor()
    {
        return backgroundColor;
    }
    
    public void setTextColor(Color textColor)
    {
        try
        {
            txtColor = textColor;
            cbMessagingEnabled.setForeground(textColor);  
        }
        catch(Exception e)
        {
            e.printStackTrace();
        }
    }
    
    public Color getTextColor()
    {
        return cbMessagingEnabled.getForeground(); 
    }
    
    /** This method is called from within the constructor to
     * initialize the form.
     * WARNING: Do NOT modify this code. The content of this method is
     * always regenerated by the Form Editor.
     */
    private void initComponents() {//GEN-BEGIN:initComponents
        DescriptorPanel = new javax.swing.JPanel();
        cbMessagingEnabled = new javax.swing.JCheckBox();

        setLayout(new java.awt.BorderLayout());

        setBackground(new java.awt.Color(100, 120, 170));
        DescriptorPanel.setLayout(new java.awt.BorderLayout());

        DescriptorPanel.setBackground(new java.awt.Color(100, 120, 170));
        cbMessagingEnabled.setBackground(new java.awt.Color(100, 120, 170));
        cbMessagingEnabled.setFont(new java.awt.Font("Arial", 1, 12));
        cbMessagingEnabled.setSelected(true);
        cbMessagingEnabled.setText("Enable Messaging Support");
        cbMessagingEnabled.setMnemonic('n');
        DescriptorPanel.add(cbMessagingEnabled, java.awt.BorderLayout.CENTER);

        add(DescriptorPanel, java.awt.BorderLayout.CENTER);

    }//GEN-END:initComponents
    
    
    // Variables declaration - do not modify//GEN-BEGIN:variables
    private javax.swing.JPanel DescriptorPanel;
    private javax.swing.JCheckBox cbMessagingEnabled;
    // End of variables declaration//GEN-END:variables
    
}
