...
public class ErrorImagesPanel extends EAPanel {
	...
	try
	{
		...
		{
			ProjectManager.putTempNoFileWrite("errBGImgPath", new File(tfSplashImage.getText()).toURL().toString());
		}
		else
		{
			ProjectManager.putTempNoFileWrite("errBGImgPath", "");
		}
	}
	catch(Exception ee)
	{
	...
	try
	{
            if(ProjectManager.get("errBGImgPath")!=null)
            {
                    if(ProjectManager.get("errBGImgPath").equalsIgnoreCase("")==false)
                    {
                        try
                        {
                            tfSplashImage.setText(new File(new URL(ProjectManager.get("errBGImgPath")).getFile()).getCanonicalPath());
                        }
                        catch(Exception e)
                        {
...
// End File Locations
    ...
    private void initComponents() {//GEN-BEGIN:initComponents
        ...
        ProjectLookAndFeelImages.setBorder(new javax.swing.border.TitledBorder(new javax.swing.border.EtchedBorder(new java.awt.Color(198, 226, 253), new java.awt.Color(96, 110, 145)), "Error Dialog Background Image", javax.swing.border.TitledBorder.DEFAULT_JUSTIFICATION, javax.swing.border.TitledBorder.DEFAULT_POSITION, new java.awt.Font("Arial", 1, 12)));
        ...
        ImageDescription.setText("The error dialog background image preferred size is 320 pixels wide by 140 pixels high.");
...
//TODO Code goes here.
    ...
    private void SplashImageDefaultButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_SplashImageDefaultButtonActionPerformed
		try
		{
             	ProjectManager.put("errBGImgPath", new URL(new File(System.getProperty("user.dir")).toURL().toString() + "/lib/images/msgbg.png").toString());
			tfSplashImage.setText(new File(new URL(ProjectManager.get("errBGImgPath")).getFile()).getCanonicalPath());
			tfSplashImage.setCaretPosition(0);
		}
		catch(Exception e)
	...
	try
	{
		int result = imageFileChooser.showOpenDialog(this); 
        	...
        	{ 
             	ProjectManager.put("errBGImgPath", imageFileChooser.getSelectedFile().toURL().toString());
			tfSplashImage.setText(new File(new URL(ProjectManager.get("errBGImgPath")).getFile()).getCanonicalPath());
			tfSplashImage.setCaretPosition(0);
		}
	}
