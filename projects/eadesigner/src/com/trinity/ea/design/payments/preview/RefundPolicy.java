...
import com.trinity.ea.design.payments.preview.gui.swing.EAScrollBarUI;
...
public class RefundPolicy extends javax.swing.JDialog {
	...
	private int windowHeight = 160;
	  ...
	  private boolean isRefundPreview = false;
...
try
{
    ...
    if(isImageButton==true)
    {
	  ...
	  else
	  {
        ...
        {
        	setTitle(replaceUIExpressions(ProjectManager.get("refundPolicyTitle")));
	  }
	  ...
	  Policy.setCaretPosition(0);
		...
		if(ProjectManager.get("refundPolicyButtonTextColor")!=null)
		{
			if(ProjectManager.get("refundPolicyButtonTextColor").equalsIgnoreCase("")==false)
			{
				Object[] tmpArray = getStringArraysFromString(ProjectManager.get("refundPolicyButtonTextColor"));
    ...
    private void initComponents(boolean isImgButton) {//GEN-BEGIN:initComponents
	  ...
	  Policy.setLineWrap(true);
		if(ProjectManager.get("refundPolicy")!=null)
		{
	  		...
	  		Policy.setText(replaceUIExpressions(ProjectManager.get("refundPolicy").replaceAll("\\\\r\\\\n", System.getProperty("line.separator"))));         
		} 
		...
		setResizable(false);
		if(ProjectManager.get("refundPolicyTextFont")!=null)
		{
			Object[] tmpArray = getStringArraysFromString(ProjectManager.get("refundPolicyTextFont"));
		...
		if(ProjectManager.get("refundPolicyOKBtnText")!=null)
		{
        if(isImageButton==true)
	  {
        	btnImageOK.setText(replaceUIExpressions(ProjectManager.get("refundPolicyOKBtnText")));
	  }
	  else
	  {
        	btnOK.setText(replaceUIExpressions(ProjectManager.get("refundPolicyOKBtnText")));
	  }
	  }
	  else
	  {
	  ...
	  if(isImageButton==true)
	  {
	  if(ProjectManager.get("refundPolicyOKBtnMnemonic")!=null)
	  {
	  	if(ProjectManager.get("refundPolicyOKBtnMnemonic").equalsIgnoreCase("")==false)
	  	{
		      btnImageOK.setMnemonic(ProjectManager.get("refundPolicyOKBtnMnemonic").charAt(0));
		}
	  }
	  }
	  else
	  ...
	  if(ProjectManager.get("refundPolicyOKBtnMnemonic")!=null)
	  {
	  	if(ProjectManager.get("refundPolicyOKBtnMnemonic").equalsIgnoreCase("")==false)
	  	{
	      	btnOK.setMnemonic(ProjectManager.get("refundPolicyOKBtnMnemonic").charAt(0));
		}
	  }
	  }
if(isImageButton==true)
...
if(ProjectManager.get("refundPolicyDialogSize")!=null)
{
	if(ProjectManager.get("refundPolicyDialogSize").equalsIgnoreCase("")==false)
	{	
		Object[] tmpArray = getStringArraysFromString(ProjectManager.get("refundPolicyDialogSize"));
...
//
   ...
   public void setBackgroundColor(Color BGColor)
   {
    ...
    public RefundPolicy setIsRefundPreview(boolean isRefundPrev)
    {
	isRefundPreview=isRefundPrev;
	return this;
    }
