                           ...
                           if(EncryptedUpdateReader.get("locale").equalsIgnoreCase("true")==true)
                           {
                                if(EncryptedUpdateReader.get("delnalocale")!=null)
                                {
                                    if(EncryptedUpdateReader.get("delnalocale").equalsIgnoreCase("true")==true)
                                    {
                                         ...
                                         {
                                             if(EncryptedUpdateReader.get("update_" + theLocale.getLanguage())!=null)
                                             {
                                                 //Load Localized
								 updateFileRetCode = FileDownloader.downloadFile(EncryptedUpdateReader.get("update_" + theLocale.getLanguage()), this.directory); 
                                            }
                                             else
                                             {
                                                 //Load update
								 updateFileRetCode = FileDownloader.downloadFile(EncryptedUpdateReader.get("update"), this.directory); 
                                             }
                                         }
                                         else
                                         {
                                             if(EncryptedUpdateReader.get("update_" + new Locale(theLocale.getLanguage(), "", "").getLanguage())!=null)
                                             {
                                                 //Load Localized
								 updateFileRetCode = FileDownloader.downloadFile(EncryptedUpdateReader.get("update_" + new Locale(theLocale.getLanguage(), "", "").getLanguage()), this.directory);
                                             }   
                                             else
                                             {
                                                 //Load update
